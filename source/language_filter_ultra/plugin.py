...
"""
    ...
    Copyright:
        Copyright (C) 2021 Josh.5 <jsunnex@gmail.com>, senorsmartypants@gmail.com, yajrendrag@gmail.com
...
from language_filter_ultra.lib.ffmpeg import StreamMapper, Probe, Parser
from language_filter_ultra.lib.tmdb_original import get_original_language

# Configure plugin logger
logger = logging.getLogger("Unmanic.Plugin.language_filter_ultra")
...
class Settings(PluginSettings):
    settings = {
        ...
        "subtitle_languages":         '*',
        "keep_undefined":             True,
        "discard_commentary":         True,
        "fail_safe":                  True,
        "reorder_kept":               True,
        "prefer_2_or_mc":             "stereo",
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form_settings = {
            "audio_languages": {
                "label": "Comma-delimited list of 3-letter audio languages to keep",
            },
            "subtitle_languages": {
                "label": "Comma-delimited list of 3-letter subtitle languages to keep: enter '*' to keep all",
            },
            "keep_undefined":	{
                "label": "Keep undefined/unknown audio streams",
            },
            "discard_commentary":   {
                "label": "Discard commentaries streams regardless of language",
            },
            "fail_safe":   {
                "label": "Prevent deletion of all audio streams when stars align",
            },
            "reorder_kept":   {
                "description": "Re-order kept audio streams: languages to keep will be first; set default=1 for first audio stream",
            ...
            "keep_original_audio":   {
                ...
                "description": "Will perform TMDB lookup and add original audio language tag to the list of audio languages to keep",
            },
    ...
    def __set_audio_default_prefs_form_settings(self):
        values = {
            "description": "Prefer to keep multi-channel or stereo when file has multiple streams of the same language to keep?",
            "label":      "Prefered channels",
            "input_type": "select",
            "select_options": [
                {
                    "value": "mc",
                    "label": "Set multi-channel as default (5.1 etc)",
                },
                {
                    "value": "stereo",
                    "label": "Set stereo as default",
                },
            ],
        }
...
def kept_streams(settings):
    ...
    kc = not settings.get_setting('discard_commentary')
    if kc is None:
        kc = not settings.settings.get('discard_commentary')
    fs = settings.get_setting('fail_safe')
    if fs is None:
        fs = settings.settings.get('fail_safe')

    return 'kept_streams=audio_langauges={}:subtitle_languages={}:keep_undefined={}:discard_commentary={}:fail_safe={}'.format(al, sl, ku, kc, fs)
...
def keep_languages(mapper, ct, language_list, streams, keep_undefined, discard_commentary):
    ...
    streams_list = [streams[i]["tags"]["language"] for i in range(0, len(streams)) if "codec_type" in streams[i] and streams[i]["codec_type"] == ct and "tags" in streams[i] and "language" in streams[i]["tags"] and
                    (codec_type == 's' or discard_commentary == True or (discard_commentary == False and ("codec_type" in streams[i] and streams[i]["codec_type"] == ct and "tags" in streams[i] and ("title" in streams[i]["tags"] and
...
def keep_undefined(mapper, streams, discard_commentary):
    if discard_commentary:
...
def reorder_audio_streams(stream_map, mapper, prefer_2_or_mc, ffmpeg_args, probe_streams, def_lang):
    ...
    if len(matched_langs) > 1:
        ...
        if (prefer_2_or_mc == 'stereo' and channels >= 2) or (prefer_2_or_mc == 'mc' and channels <= 2):
...
def on_worker_process(data, task_data_store=None, file_metadata=None):
    ...
    discard_commentary = settings.get_setting('discard_commentary')
    ...
    if not file_streams_already_kept(settings, data.get('original_file_path'), file_metadata=file_metadata):
        ...
        if mapper.same_streams_or_no_work(probe_streams, keep_undefined_lang_tags):
            logger.debug("File '{}' only has same streams as keep configuration specifies OR otherwise does not require any work to keep ony specified streams - so, does not contain streams that require processing.".format(abspath))
        elif streams_need_processing or (keep_all_audio and not discard_commentary):
            ...
            if not keep_all_audio:
                keep_languages(mapper, 'audio', alcl_value, probe_streams, keep_undefined_lang_tags, discard_commentary)
                def_lang = alcl_value
                ...
                if discard_commentary:
                    ...
                    # 1. Safely filter for audio streams
                    astreams = [probe_streams[i]["index"] for i in range(len(probe_streams)) if probe_streams[i].get("codec_type") == 'audio']

                    # 2. Safely check for 'title' inside 'tags', defaulting to an empty string if missing
                    audio_streams_to_map = [
                        astreams[a] for a, i in enumerate(astreams) 
                        if "commentary" not in probe_streams[i].get("tags", {}).get("title", "").lower()
                    ]

                    # 3. Append mappings to the mapper
                    for i in range(len(audio_streams_to_map)):
                        mapper.stream_mapping += ['-map', f"0:a:{i}"]

            if settings.get_setting('subtitle_languages') != '*':
                keep_languages(mapper, 'subtitle', settings.get_setting('subtitle_languages'), probe_streams, keep_undefined_lang_tags, discard_commentary)

            # keep undefined language streams if present
            if keep_undefined_lang_tags:
                keep_undefined(mapper, probe_streams, discard_commentary)
